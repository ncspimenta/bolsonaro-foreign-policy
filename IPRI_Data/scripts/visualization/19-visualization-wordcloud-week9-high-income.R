# ─────────────────────────────────────────────────────────────────────────────
# Script: 19-visualization-wordcloud-week9-high-income.R
# Generates a wordcloud of responses to open-ended questions
# (P01 and P02) from Week 9 of the IPRI survey, filtered to include
...
respostas_p9 <- bd9 %>%
  select(`P2_Out#1`)  # P1_Out#1 is not present in BD 9
...
tokens <- texto_completo %>%
  ...
  filter(
    ...
    !word %in% c("outros", "anotar", "não", "respondeu", "nenhuma", "sabe", "ler",
                 "nh", "pra", "nao", "sobre", "sao", "so", "d", "ser", "nr", "divuylgacao", "ultir"),
    str_detect(word, "[a-z]")
  )
